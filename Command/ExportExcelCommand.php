...
class ExportExcelCommand extends ContainerAwareCommand implements SingleRunCommandInterface  {
...
//            ->addOption('yell', null, InputOption::VALUE_NONE, 'If set, the task will yell in uppercase letters')
    ...
    protected function execute(InputInterface $input, OutputInterface $output) {
        ...
        $result = $this->dm->createQueryBuilder('IbtikarGlanceDashboardBundle:Export')
                        ...
                        ->field('type')->equals(Export::VISITORS)
                        ->getQuery()->execute();
...
//    }
    ...
    private function getVisitorList($export, $queryCount=false, $skip=0, $limit=1000) {
        $queryBuilder = $this->dm->createQueryBuilder('IbtikarGlanceUMSBundle:Visitor')->field('deleted')->equals(false);
        ...
        if (isset($params['nickName']) && !empty($params['nickName'])) {
            $queryBuilder = $queryBuilder->field('nickName')->equals(new \MongoRegex(('/' . preg_quote(trim($params['nickName'])) . '/i')));
        }
        if (isset($params['email']) && !empty($params['email'])) {
            $queryBuilder = $queryBuilder->field('email')->equals(new \MongoRegex(('/' . preg_quote(trim($params['email'])) . '/i')));
        }
...
//        $this->printMemoryUsage();
        foreach ($result as $export) {
            ...
            $count = $this->getVisitorList($export, true);
            $skip = 0;
            $index = 1;

            while ($skip < $count) {
                $queryBuilder = $this->getVisitorList($export, false, $skip);
                $result = $queryBuilder->getQuery()->execute();

                if (is_null($export->getFields())) {
                    $createExcel->setFields(array('createdAt', 'email', 'nickName'));
                } else {
                ...
                $fileName = $index.'-'.$export->getName() . $this->currentDate . '.' .$export->getExtension();
            ...
            $this->createZip($export->getName(), $export->getExtension());
//            $this->printMemoryUsage();
            $this->nextState($export);
    ...
    private function sendMail($user, $filename) {
        ...
        $filesavename = $this->container->get('translator')->trans('excel exported file', array(), 'visitor');
        $body = str_replace(
                array(
            '%fullname%',
            '%smallMessage%',
