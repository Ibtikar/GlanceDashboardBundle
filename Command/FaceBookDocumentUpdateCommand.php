...
class FaceBookDocumentUpdateCommand extends ContainerAwareCommand {
    ...
    protected function execute(InputInterface $input, OutputInterface $output) {
        ...
        if (count($documents) > 0) {
            ...
            foreach ($documents as $document) {
                $data = array('id' => $this->getContainer()->get('router')->generate('ibtikar_goody_frontend_'.trim($document->getType()).'_view', array('slug' => $document->getDocument()->getSlug(), '_locale'=> 'ar'), true), 'scrape' => TRUE);
