...
function populateData() {
    ...
    $('#media-list-target-right-activity').find('tr').each(function (order) {
        var imgSrc = $(this).find('img').attr('src');
        mediaContainer.push({
            id: $(this).attr('id'),
            vid: $(this).attr('video-id'),
...
function setUploadedImagesCount() {
    ...
    $('.dev-image-count-activity').html($('#media-list-target-right-activity .icon-image2').length);
}

function setUploadedVideosCount() {
    $('.dev-video-count').html($('#media-list-target-right .icon-video-camera').length);
    $('.dev-video-count-activity').html($('#media-list-target-right-activity .icon-video-camera').length);
}
...
function addImageToSortViewActivity(selector, media) {
    selector.append(
            imageSortTemplateActivity
            .replace(/%image-id%/g, media.id)
            .replace(/%image-name%|%title%/g, media.path ? media.path : media.vid)
            .replace(/%image-src%/g, media.type == "image" ? '/' + media.imageUrl + '?flushCache=' + encodeURIComponent(new Date().getTime() + Math.random()) : media.imageUrl)
            .replace(/%image-caption%/g, quoteattr(media.caption))
...
function appendResults(response) {
    $('.dev-google-upload').removeAttr('ajax-running', true);
    $('.dev-google-upload').find('.icon-spinner6.spinner.position-right').remove();
    ...
    if (typeof imageSearch.result.items !== "undefined") {
        ...
        for (var i = 0; i < results.length; i++) {
            ...
            if (result.image.height < 600 || result.image.width < 1170 || result.image.byteSize > 4194304) {
// || checkExtensionValid(image_link[0])
            ...
            imgContainer = '<div class="col-lg-2 col-sm-4"><div class="thumbnail"><div class="thumb">'
                    ...
                    + '<label><input type="radio" name="stacked-radio-left" '+disabled+' data-url="' + result.link + '" class="styled" >' + result.title.slice(0, 10) + '</label>'
                    + '</div></div></div></div>';
...
$(document).on('click', '.dev-google-upload', function () {
    ...
    if ($('.dev-google-upload').attr('ajax-running')) {
        return;
    }
    $('.dev-google-upload').attr('ajax-running', true);
    $('.dev-google-upload').append('<i class="icon-spinner6 spinner position-right"></i>');
    if (extension == 'gif') {
        var selectedImage = [];
    ...
    $.ajax({
        ...
        success: function (data) {
            for (var i = 0; i < data.images.length; i++) {
                if (data.images[i].coverPhoto) {
                    addImageToSortView($('.filesUploaded table tbody'), data.images[i]);
                } else {
                    addImageToSortViewActivity($('.filesActivityUploaded table tbody'), data.images[i]);
...
function isValidKey(keyCode) {
    ...
    if ($.inArray(keyCode, invalidKeysCodes) !== -1)
        return false;
}
...
//            $('.dev-youtube-submit').attr('disabled', 'disabled');
            if ($('.dev-youtube-submit').attr('ajax-running')) {
                return;
            }
            $('.dev-youtube-submit').attr('ajax-running', true);
            $('.dev-youtube-submit').append('<i class="icon-spinner6 spinner position-right"></i>');

            populateData();

            $.ajax({
                url: youtubeUploadVideo + '?imageType=' + name,
                method: "POST",
                data: {
                ...
                success: function (data) {
                    if (data.status === 'success') {
                        ...
                        $(data.video).each(function () {
                            if (this.coverPhoto) {
                                ...
                                addImageToSortViewActivity($('.filesActivityUploaded table tbody'), this);
                    ...
                    $('.dev-youtube-submit').removeAttr('ajax-running', true);
                    $('.dev-youtube-submit').find('.icon-spinner6.spinner.position-right').remove();
                }
            });
        }
    });
  ...
  $('input').bind('keypress keydown', function(e){
    ...
    $('#GoogleImportImg-modal').on('hidden.bs.modal', function (e) {
        $('#dev-search-gimage-box,#dev-search-gimage-box-small,#dev-search-gimage-box-small-activity').val("");
    });
...
//            }
    ...
    $(document).on('click', '.dev-videourl-submit', function () {
        ...
        var videoId = YT.getIDFromURL(videoUrl);
...
//////////////////////////////////////////////////////////////////////
    ...
    //activity code
    $(document).on('click', '.dev-upload-activity-img', function () {
        $('input.cropit-image-input').val('');
        $('.cropit-preview').removeClass('cropit-image-loaded');
        $('.cropit-preview-image').attr('src', '');
        name = 'activityPhoto';
    ...
    $(document).on('click', '#search-img-btn-small-activity', function () {
        if ($('#dev-search-gimage-box-small-activity').val().trim() != "") {
            $('#dev-search-gimage-box').val($('#dev-search-gimage-box-small-activity').val());
            sendRequestToGoogleImageApi();
            name = 'activityPhoto';
        } else {
            $('#dev-search-gimage-box-small-activity').trigger('keyup');
        }
    });
    $(document).on('click', '#search-vid-btn-small-activity', function () {
        if ($('#dev-search-gvideo-box-small-activity').val().trim() != "") {
            $('#dev-search-gvideo-box').val($('#dev-search-gvideo-box-small-activity').val());
            $('#search-vid-btn').trigger('click');
            name = 'activityPhoto';
    ...
    $(document).on('click', '.dev-imageurl-submit-activity', function () {
        var errorContainer = $('.dev-recipe-imgeUrl-error');
        var obj = $('input.dev-recipe-imgeUrl-activity').val();
        var imageSrc = $.trim(obj);
        var errorContainer = $('.dev-recipe-imgeUrl-error');
        if (imageSrc.length > 0) {
            var extension = encodeURI(imageSrc).substring((imageSrc).lastIndexOf('.') + 1);
...
//            }
            ...
            $('.dev-recipe-imgeUrl-activity').trigger('keyup');
        }
    });
    $(document).on('click', '.dev-videourl-submit-activity', function () {
        ...
        var obj = $('input.dev-recipe-videoUrl-activity').val();
        var videoUrl = $.trim(obj);
        ...
        if (!valid_youtubeUrl(videoUrl)) {
            showNotificationMsg(messages.wrongURL, '', 'error');
            $('.dev-recipe-videoUrl-activity').val('');
            return;
        }
        ...
        ytXhr = $.ajax({
            ...
            success: function (data) {
                if (data == 'error') {
                    ...
                    $('.dev-videourl-submit-activity').attr('disabled', 'disabled');
                    var videos = [];
                    videos[0] = videoId;
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            $('.dev-recipe-videoUrl-activity').val('');
                            $('.dev-videourl-submit-activity').removeAttr('disabled');
                        }
                    });
